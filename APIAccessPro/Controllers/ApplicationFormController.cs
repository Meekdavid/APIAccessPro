...
namespace APIAccessPro.Controllers
{
    ...
    public class ApplicationFormController : Controller
    {
        ...
        public async Task<ActionResult<ApplicationFormResponse>> RetrieveForms()
        {
            ...
            try
            {
                var response = await _formAccess.RetrieveFormsAsync(Utils.SelectQuery);
                response.Logs.AddToLogs(ref logs);//Add returned logs to list of logs
                logBuilder.ToString().AddToLogs(ref logs);
                logBuilder.Clear();//Add the logs from the stringbuilder and empty the stringbuilder
        ...
        {
            string methodName = "AmendAddedQuestions", classAndMethodName = $"{className}.{methodName}";
            var logs = new List<Log>();
            var returnHttpStatusCode = Utils.HttpStatusCode_Ok;
